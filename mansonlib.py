	...
	def RPreset(self,index):
		if type(self.sp) != type(None):
			...
			if isinstance(index,int):
				...
				return response
		return False
	# The get mode just decodes a serial message
	def GetOutVolt(self):
		if type(self.sp) != type(None):
			self.sp.write("GETD\r".encode('ascii'))
			response = self.sp.readline().decode('ascii')
...
# class basically gives some comments on the older classes
...
#	pass
# There have been changes in the inheritance principle from python3 onwards
# the order is changed to support a valid MRO
class NTP(HCS, InstrumentInterface):
	def SetOutputVoltage(self,volt):
		if type(self.sp) != type(None):
			# check if input is some kind of number
			if isinstance(volt,(int,float)):
				# the number passed must be bigger than 1, voltages smaller than 1 won't work.
